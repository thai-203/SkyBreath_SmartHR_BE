...
export class RequestsRepository {
  ...
  get repository() {
    if (!this._repository)
      this._repository = AppDataSource.getRepository(RequestEntity);
    return this._repository;
  }

  get approvalRepo() {
    if (!this._approvalRepo)
      this._approvalRepo = AppDataSource.getRepository(
  ...
  get attachmentRepo() {
    if (!this._attachmentRepo)
      this._attachmentRepo = AppDataSource.getRepository(
  ...
  get otdRepository() {
    if (!this._otdRepo)
      this._otdRepo = AppDataSource.getRepository(OvertimeRequestDetailEntity);
  ...
  async findMyRequests({
    ...
    const query = this.repository
      ...
      .andWhere('(r.employeeId = :empId OR r.createdByEmployeeId = :empId)', {
        empId: employeeId,
      });

    if (status) query.andWhere('r.status = :status', { status });
    if (requestTypeId)
      query.andWhere('r.requestTypeId = :requestTypeId', { requestTypeId });
  ...
  async findAll({
    ...
    if (requestTypeId)
      query.andWhere('r.requestTypeId = :requestTypeId', { requestTypeId });
    if (employeeId)
      query.andWhere('r.employeeId = :employeeId', { employeeId });
    if (startDate) query.andWhere('r.startDate >= :startDate', { startDate });
    if (endDate) query.andWhere('r.endDate <= :endDate', { endDate });
      ...
      .andWhere(
        new Brackets((qb) => {
          qb.where('requestGroup.code IN (:...excuseGroupCodes)', {
            excuseGroupCodes: [
              RequestGroupCode.LATE_EARLY,
              RequestGroupCode.ATTENDANCE_CORRECTION,
            ],
          }).orWhere('r.requestTypeId = :legacyExcuseTypeId', {
            legacyExcuseTypeId: 2,
          });
        }),
      );
  ...
  async findOvertimeDetailLinesForGroup({
    ...
    if (search) {
      shared.sql +=
        ' AND (e.full_name LIKE ? OR e.employee_code LIKE ? OR r.request_code LIKE ?)';
      const s = `%${search}%`;
    ...
    if (year != null && month != null) {
      detailDateSql =
        ' AND YEAR(otd.work_date) = ? AND MONTH(otd.work_date) = ?';
      ...
      headerDateSql =
        ' AND r.start_date IS NOT NULL AND YEAR(r.start_date) = ?';
    ...
    const detailIds = lineRows
      .map((row) => row.detail_id)
      .filter((id) => id != null);
    const requestIds = [
      ...new Set(lineRows.map((row) => row.request_id).filter(Boolean)),
    ];
  ...
  async getTodayOvertime(employeeId, date) {
    const otDetailRepo = AppDataSource.getRepository(
      OvertimeRequestDetailEntity,
    );

    // 1. Lấy OT detail hôm nay
    const details = await otDetailRepo
      ...
      .andWhere('requestGroup.code = :group', { group: 'OVERTIME' })
      .andWhere('detail.workDate = :date', { date })
      .andWhere('request.isDeleted = false')
      .getMany();

    // 2. Lấy request header fallback
    ...
    const headers = requests.filter(
      (r) =>
        r.requestGroup?.code === 'OVERTIME' &&
        r.startDate === date &&
        !details.some((d) => d.requestId === r.id),
    );
    ...
    const items = [
      ...
      ...headers.map((r) => ({
        ...
        startTime: r.startTime,
        endTime: r.endTime,
        totalHours: Number(r.quantity || 0),
        request: r,
      })),
    ];
  ...
  async findUsedRequests({
    ...
    const query = this.repository
      ...
      .andWhere('r.startDate <= :periodEnd', {
        periodEnd: periodEnd.toISOString().slice(0, 10),
      })
      .andWhere('r.endDate >= :periodStart', {
