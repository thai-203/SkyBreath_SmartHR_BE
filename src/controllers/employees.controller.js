...
export class EmployeesController {
        ...
        try {
            ...
            if (req.files) {
                if (req.files['frontIdCard']) {
                    createDto.frontIdCardFilePath = req.files['frontIdCard'][0].path.replace(/\\/g, '/');
                }
                if (req.files['backIdCard']) {
                    createDto.backIdCardFilePath = req.files['backIdCard'][0].path.replace(/\\/g, '/');
                }
            }

            const result = await this.employeesService.create(createDto);
            ResponseUtil.sendResponse(res, AppMessages.Success.Employee.CREATED, result);
        } catch (error) {
            next(error);
        }
    };
    ...
    findOne = async (req, res, next) => {
        try {
            const employee = await this.employeesService.findById(parseInt(req.params.id));
    ...
    getMetadata = async (req, res, next) => {
        try {
            ...
            ResponseUtil.sendResponse(res, AppMessages.Success.General?.RETRIEVED || 'Data retrieved successfully', metadata);
        } catch (error) {
            next(error);
        }
    };
    ...
    list = async (req, res, next) => {
        try {
            ...
            ResponseUtil.sendResponse(res, AppMessages.Success.Employee.RETRIEVED_ALL, list);
        } catch (error) {
            next(error);
        }
    };
    ...
    update = async (req, res, next) => {
        try {
            const updateDto = plainToInstance(UpdateEmployeeDto, req.body, { excludeExtraneousValues: true });

            // Handle uploaded files
            if (req.files) {
                if (req.files['frontIdCard']) {
                    updateDto.frontIdCardFilePath = req.files['frontIdCard'][0].path.replace(/\\/g, '/');
                }
                if (req.files['backIdCard']) {
                    updateDto.backIdCardFilePath = req.files['backIdCard'][0].path.replace(/\\/g, '/');
                }
            }
    ...
    getValidationData = async (req, res, next) => {
        try {
            ...
            ResponseUtil.sendResponse(res, AppMessages.Success.General?.RETRIEVED || 'Data retrieved successfully', data);
        } catch (error) {
            next(error);
        }
    };
