...
export class JobGradesService {
    ...
    async create(createDto) {
        const existing = await this.jobGradesRepository.findByName(createDto.gradeName);
        if (existing) {
    ...
    async findByDepartment(departmentId) {
        return this.jobGradesRepository.findByDepartment(departmentId);
    }
    ...
    async update(id, updateDto) {
        ...
        if (updateDto.gradeName) {
            const existing = await this.jobGradesRepository.findByName(updateDto.gradeName);
    ...
    async exportExcel() {
        const [jobGrades] = await this.jobGradesRepository.findAll({ limit: 10000, page: 1 });

        const data = jobGrades.map((jg, index) => ({
            index: index + 1,
            gradeName: jg.gradeName,
            department: jg.department?.departmentName || '',
