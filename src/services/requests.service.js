...
import {
  ...
  ApproverType,
  RequestGroupCode,
} from '../common/enums/request.enum.js';
...
import {
  ...
  getRequestedHours,
} from '../common/helpers/working-days.helper.js';
...
export class RequestsService {
  ...
  async saveDraft(body, reqUser) {
    ...
    if (targetEmployeeId !== creatorEmployee.id) {
      if (!reqUser.permissions.includes('REQUEST_CREATE_FOR_OTHERS')) {
        throw new ForbiddenException(
          'Bạn không có quyền tạo đơn cho người khác',
        );
      }
    }
    ...
    if (!requestType || requestType.status !== 'ACTIVE') {
      throw new BadRequestException(
        'Loại đơn không tồn tại hoặc không còn hoạt động',
      );
    }
    ...
    const reqGroup = await this.requestGroupsRepo.findById(
      requestType.requestGroupId,
    );
    const isOvertime =
      reqGroup &&
      (/overtime|tăng ca/i.test(reqGroup.code || '') ||
        /overtime|tăng ca/i.test(reqGroup.name || ''));
    ...
    if (startDate && endDate) {
      ...
      if (overlaps.length > 0) {
        throw new BadRequestException(
          'Thời gian lưu đơn bị trùng lặp với một đơn khác đang có hiệu lực trong hệ thống.',
        );
      }
      ...
      if (reqUnit === 'DAY') {
        estimatedQuantity = await countWorkingDays(
          startD,
          endD,
          targetEmployeeId,
          AppDataSource,
        ...
        if (startTime && endTime) {
          estimatedQuantity = await getRequestedHours(
            startDate,
            endDate,
            startTime,
            endTime,
          ...
          while (cursor <= endD) {
            estimatedQuantity += await getWorkingHoursForDay(
              cursor,
              targetEmployeeId,
              AppDataSource,
              isOvertime,
      ...
      if (isOvertime) {
        ...
        while (cursor <= endD) {
          ...
          if (reqUnit === 'DAY') {
            dayHours = await getWorkingHoursForDay(
              cursor,
              targetEmployeeId,
              AppDataSource,
              true,
            ...
            dayHours = await getRequestedHours(
              currentStr,
              currentStr,
              startTime,
              endTime,
    ...
    if (requestId) {
      ...
      if (existing.status !== RequestStatus.DRAFT) {
        throw new BadRequestException(
          'Chỉ có thể chỉnh sửa đơn ở trạng thái nháp',
        );
      }
    ...
    if (isOvertime) {
      ...
      if (detailsToSave.length > 0) {
        const entries = detailsToSave.map((d) => ({
          ...d,
          requestId: savedRequest.id,
        }));
        await this.repo.otdRepository.save(
          this.repo.otdRepository.create(entries),
        );
      }
    }
  ...
  async getQuotaStatus(requestTypeId, employeeId, excludeRequestId = null) {
    ...
    const isOvertime =
      ...
      (/overtime|tăng ca/i.test(requestType.requestGroup.code || '') ||
        /overtime|tăng ca/i.test(requestType.requestGroup.name || ''));
    ...
    if (
      !policy ||
      !policy.maxQuantity ||
      parseFloat(policy.maxQuantity) === 0 ||
      policy.isUnlimited
    ) {
      return {
        hasQuota: false,
        ...
        trackingCycle: policy?.trackingCycle || 'YEAR',
      };
    }
    ...
    for (const req of usedRequests) {
      ...
      if (unit === 'DAY') {
        ...
        if (req.startTime && req.endTime) {
          const reqHours = await getRequestedHours(
            req.startDate,
            req.endDate,
            req.startTime,
            req.endTime,
          ...
          while (cursor <= clipEnd) {
            const hoursInDay = await getWorkingHoursForDay(
              cursor,
              employeeId,
              AppDataSource,
              isOvertime,
    ...
    const remainingQuantity = Math.max(
      0,
      parseFloat(maxQuantity) - usedQuantity,
    );
    return {
      hasQuota: true,
  ...
  async estimateQuantity(query) {
    ...
    if (!employeeId || !startDate || !endDate || !unit) {
      throw new BadRequestException(
        'Thiếu tham số (employeeId, startDate, endDate, unit)',
      );
    }
    ...
    if (requestTypeId) {
      ...
      if (reqType?.requestGroup) {
        isOvertime =
          /overtime|tăng ca/i.test(reqType.requestGroup.code || '') ||
          /overtime|tăng ca/i.test(reqType.requestGroup.name || '');
      }
    }
    ...
    if (unit === 'DAY') {
      requestedQty = await countWorkingDays(
        reqStart,
        reqEnd,
        employeeId,
        AppDataSource,
      ...
      if (startTime && endTime) {
        requestedQty = await getRequestedHours(
          startDate,
          endDate,
          startTime,
          endTime,
        ...
        cursor.setHours(0, 0, 0, 0);
        while (cursor <= reqEnd) {
          requestedQty += await getWorkingHoursForDay(
            cursor,
            employeeId,
            AppDataSource,
            isOvertime,
  ...
  async submitRequest(requestId, body, reqUser) {
    ...
    if (request.status !== RequestStatus.DRAFT) {
      throw new BadRequestException(
        'Chỉ có thể gửi duyệt đơn ở trạng thái nháp',
      );
    }
    if (!request.startDate)
      throw new BadRequestException('Thiếu thông tin ngày bắt đầu');
    if (!request.endDate)
      throw new BadRequestException('Thiếu thông tin ngày kết thúc');
    ...
    if (overlaps.length > 0) {
      throw new BadRequestException(
        'Thời gian gửi duyệt của đơn bị trùng lặp với một đơn khác đang có hiệu lực.',
      );
    }
    ...
    try {
      ...
      if (quota.hasQuota) {
        ...
        if (requestedQty > quota.remainingQuantity) {
          const unitLabel =
            { DAY: 'ngày', HOUR: 'giờ', HALF_DAY: 'nửa ngày', TIME: 'lần' }[
              quota.unit
            ] || quota.unit;
          throw new BadRequestException(
    ...
    const workflows = await this.workflowRepo.findByGroupId(
      request.requestGroupId,
    );
    if (!workflows || workflows.length === 0) {
      throw new BadRequestException(
        'Nhóm đơn chưa được cấu hình workflow phê duyệt',
      );
    }
    ...
    try {
      ...
      if (firstLevel) {
        const approverEmp = await this.employeeRepo.findOne({
          where: { id: firstLevel.approverEmployeeId, isDeleted: false },
        });
  ...
  async cancelRequest(requestId, reqUser) {
    ...
    if (
      request.createdByEmployeeId !== employee?.id &&
    ...
    if (
      ![RequestStatus.DRAFT, RequestStatus.PENDING].includes(request.status)
    ) {
    ...
    if (request.status === RequestStatus.PENDING) {
      ...
      const anyApproved = levels.some(
        (l) => l.status === ApprovalLevelStatus.APPROVED,
      );
      if (anyApproved) {
        throw new BadRequestException(
      ...
      try {
        const currentLevel = levels.find(
          (l) => l.levelOrder === request.currentApprovalLevel,
        );
        if (currentLevel) {
          const approverEmp = await this.employeeRepo.findOne({
            where: { id: currentLevel.approverEmployeeId, isDeleted: false },
          });
          if (approverEmp?.userId) {
            const creatorName = await this._getEmployeeName(request.employeeId);
            this.notificationService.emitRemovePendingRequest(
              [approverEmp.userId],
              requestId,
            );
            await this.notificationService.createAndNotify({
  ...
  async approveRequest(requestId, reqUser) {
    ...
    const currentLevel = await this.repo.getApprovalLevel(
      requestId,
      request.currentApprovalLevel,
    );
    if (!currentLevel)
      throw new BadRequestException(
        'Không tìm thấy thông tin cấp duyệt hiện tại',
      );

    if (currentLevel.approverEmployeeId !== approver?.id) {
      throw new ForbiddenException(
        'Bạn không phải người duyệt hợp lệ cho cấp duyệt này',
      );
    }
    ...
    if (isLastLevel) {
      ...
      try {
        ...
        console.error(
          '[Notification] Lỗi gửi thông báo approve (cấp cuối):',
          err,
        );
      }
    } else {
      ...
      try {
        ...
        const nextLevel = allLevels.find(
          (l) => l.levelOrder === request.currentApprovalLevel + 1,
        );
        const approverName = approver?.fullName || 'Người duyệt';

        // → Thông báo cho cấp tiếp theo
        if (nextLevel) {
          const nextApproverEmp = await this.employeeRepo.findOne({
            where: { id: nextLevel.approverEmployeeId, isDeleted: false },
          });
          const nextApproverName =
            nextApproverEmp?.fullName || 'người duyệt tiếp theo';
        ...
        console.error(
          '[Notification] Lỗi gửi thông báo approve (cấp giữa):',
          err,
        );
      }
    }
  ...
  async rejectRequest(requestId, { comment }, reqUser) {
    ...
    try {
      ...
      for (const lvl of allLevels) {
        if (
          lvl.status === ApprovalLevelStatus.APPROVED &&
          lvl.approverEmployeeId
        ) {
          const emp = await this.employeeRepo.findOne({
            where: { id: lvl.approverEmployeeId, isDeleted: false },
          });
  ...
  async revokeApproval(requestId, { levelOrder, comment }, reqUser) {
    ...
    if (request.status !== RequestStatus.PENDING) {
      throw new BadRequestException(
        'Chỉ có thể hủy duyệt khi đơn đang trong trạng thái chờ duyệt (PENDING)',
      );
    }

    const targetLevel = await this.repo.getApprovalLevel(requestId, levelOrder);
    if (!targetLevel)
      throw new NotFoundException('Không tìm thấy cấp duyệt cần hủy');

    if (targetLevel.approverEmployeeId !== approver?.id) {
      throw new ForbiddenException(
        'Bạn chỉ có thể hủy duyệt cấp mà bạn đã phê duyệt',
      );
    }

    if (targetLevel.status !== ApprovalLevelStatus.APPROVED) {
      throw new BadRequestException(
        'Cấp duyệt này chưa được phê duyệt, không thể hủy duyệt',
      );
    }
  ...
  async getRequestDetail(requestId, reqUser) {
    ...
    if (!hasViewAll) {
      ...
      const isApprover = approvalLevels.some(
        (l) => l.approverEmployeeId === employee?.id,
      );
  ...
  async getPendingApprovals(query, reqUser) {
    ...
    const { items, total } = await this.repo.findPendingForApprover(
      employee.id,
      {
        skip,
        limit: parseInt(limit),
  ...
  async getOvertimeDetailRequests(query, reqUser) {
    const {
      ...
      page = 1,
      limit = 20,
      viewAll = false, // New parameter to override default behavior
    } = query;
    ...
    let employeeId;
    const hasViewAll = reqUser?.permissions?.includes('REQUEST_VIEW_ALL');
    // Chỉ filter theo employeeId nếu user KHÔNG có permission VIEW_ALL và KHÔNG request viewAll
    if (!hasViewAll && !viewAll) {
      const employee = await this._getEmployeeByUserId(reqUser.id);
      employeeId = employee?.id;
    }

    const otGroup = await this.requestGroupsRepo.findByCode(
      RequestGroupCode.OVERTIME,
    );
      ...
      workflows.map(async (wf) => {
        ...
        return {
          ...
          approverEmployee: approver
            ? { id: approver.id, fullName: approver.fullName }
            : null,
          resolved: !!approver,
        };
      }),
