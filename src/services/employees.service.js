...
import { mailService } from '../common/utils/mail.service.js';
...
export class EmployeesService {
    ...
    async create(createDto) {
        if (createDto.personalEmail) {
            const existing = await this.employeesRepository.findByField('personalEmail', createDto.personalEmail);
            if (existing) {
        ...
        if (createDto.phoneNumber) {
            const existing = await this.employeesRepository.findByField('phoneNumber', createDto.phoneNumber);
            if (existing) {
        ...
        if (createDto.nationalId) {
            const existing = await this.employeesRepository.findByField('nationalId', createDto.nationalId);
            if (existing) {
        ...
        if (createDto.companyEmail) {
            ...
            let user = await userRepo.findOne({ where: { email: createDto.companyEmail } });
            ...
            if (!user) {
                ...
                const role = await roleRepo.findOne({ where: { roleName: 'EMPLOYEE' } });
                if (role) {
                ...
                await mailService.sendAccountInfo(user.email, createDto.fullName, user.username, 'password123');
            }
    ...
    async getMetadata() {
        ...
        const positions = await AppDataSource.getRepository(PositionEntity).find({ where: { isDeleted: false }, order: { positionName: 'ASC' } });
        const jobGrades = await AppDataSource.getRepository(JobGradeEntity).find({ where: { isDeleted: false }, order: { gradeName: 'ASC' } });
    ...
    async exportExcel() {
        const { items } = await this.employeesRepository.findAll({ limit: 10000, page: 1 });
        ...
        const data = items.map((e, index) => ({
            ...
            dateOfBirth: e.dateOfBirth ? new Date(e.dateOfBirth).toLocaleDateString('vi-VN') : '',
            nationalId: e.nationalId || '',
            nationalIdIssuedDate: e.nationalIdIssuedDate ? new Date(e.nationalIdIssuedDate).toLocaleDateString('vi-VN') : '',
            ...
            joinDate: e.joinDate ? new Date(e.joinDate).toLocaleDateString('vi-VN') : '',
            officialStartDate: e.officialStartDate ? new Date(e.officialStartDate).toLocaleDateString('vi-VN') : '',
